# windows_remote_bootstrap_v8.ps1
# Run as Administrator — Tailscale (NoState fix) + OpenSSH + Hermes SSH key
...
# Start Tailscale service/process robustly
...
if (-not $tailscaleIp) {
  ...
  Write-Host "Opening Tailscale login..."
  & $tailscaleExe up
  Write-Host "Login/authorize Tailscale in the browser, then rerun v8."
  exit 20
}
