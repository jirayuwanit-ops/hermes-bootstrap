# windows_remote_bootstrap_v7.ps1
# Run as Administrator — Tailscale + OpenSSH via winget/Win32-OpenSSH + Hermes SSH key
...
if ($tailscaleExe) {
  Write-Host "Tailscale already installed at $tailscaleExe"
  & $tailscaleExe ip -4
} else {
...
# Print Tailscale status + IP
Write-Host "=== Tailscale Status ==="
& $tailscaleExe status
$tailscaleIp = & $tailscaleExe ip -4 2>&1 | Out-String
...
Write-Host "=== DONE ==="
Write-Host "Windows Tailscale IP: $(& $tailscaleExe ip -4)"
