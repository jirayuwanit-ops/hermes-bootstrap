# windows_remote_bootstrap_v5.ps1
# Run as Administrator — one-shot Tailscale + OpenSSH + Hermes SSH key

# 1. Admin detection + self-elevate
$isAdmin = ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
if (-not $isAdmin) {
    Write-Host "Not running as Administrator. Relaunching elevated..."
    Start-Process powershell -Verb RunAs -ArgumentList "-ExecutionPolicy Bypass -File C:\ProgramData\HermesBootstrap\hb.ps1"
    exit
}
...
if (-not $installed) {
    ...
    if (-not $installed) {
        ...
        Write-Host "   New-Item -ItemType Directory -Force C:\ProgramData\HermesBootstrap | Out-Null; iwr -Uri https://raw.githubusercontent.com/jirayuwanit-ops/hermes-bootstrap/main/windows_remote_bootstrap_v5.ps1 -OutFile C:\ProgramData\HermesBootstrap\hb.ps1; powershell -ExecutionPolicy Bypass -File C:\ProgramData\HermesBootstrap\hb.ps1"
        Write-Host "==============================================`n"
        exit 20
    }
} else {
